...
public class RedisLockAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (waitTime > 0 && leaseTime > 0) {
            isLock = redisLockUtils.tryLock(lockKey, timeUnit, waitTime, leaseTime);
        } else if (leaseTime > 0) {
            isLock = redisLockUtils.tryLock(lockKey, timeUnit, 0, leaseTime);
        } else if (waitTime > 0) {
            isLock = redisLockUtils.tryLock(lockKey, timeUnit, waitTime);
        } else {
            lock = redisLockUtils.lock(lockKey);
            isLock = Boolean.TRUE;
        }
        try {
            if (isLock) {
                ...
                log.error("获取分布式锁[异常]，lockName=" + lockKey);
                throw new RedisLockException("获取分布式锁[异常]，lockName=" + lockKey);
            }
        } finally {
